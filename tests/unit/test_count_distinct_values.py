from count_distinct_values import count_values
...
def test_count_values():
    ...
    assert len(expected_res) == len(
        res1
    ), f"expected {len(expected_res)} distinct values got {len(res1)}"
    assert isinstance(res1, dict), f"expected dict as returning type got {type(res1)}"
    for el, val in res1.items():
        assert (
            expected_res[el] == val
        ), f"expected {el} to be repeated {val} times, got {expected_res[el]}"

    res2 = count_values(list(sentence))
    assert len(expected_res) == len(
        res2
    ), f"expected {len(expected_res)} distinct values got {len(res2)}"
